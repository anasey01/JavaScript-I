...
console.log(`The last car in the inventory has a make of ${lastCar.car_make} which is model ${lastCar.car_model}`);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = [];
let modelsOfCars = (data) => {
    for(let i = 0; i < data.length; i++) {
        carModels.push(data[i].car_model);
    }
    return carModels.sort();
}
console.log(modelsOfCars(inventory));
...
console.log(yearOfCars(inventory));
...
console.log(NumOfOldCars());

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi =[];
let populateBMWandAudi = (arr) => {
...
console.log(populateBMWandAudi(inventory));
